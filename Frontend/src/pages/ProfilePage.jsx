...
const ProfilePage = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-100 to-blue-50 flex flex-col">
      {/* Top Nav Bar */}
      <div className="fixed top-0 w-full bg-white shadow-md z-10 flex justify-between items-center px-8 py-4">
        <h2 className="text-2xl font-semibold text-[#c7899e]">My Profile</h2>
        <div className="flex space-x-4">
          {[
            ...
            <button key={name} className={`flex items-center py-2 px-4 rounded-md text-lg transition-all duration-300 transform hover:scale-105 ${ activeTab === tab
                  ? "bg-[#c7899e] text-white shadow-md"
                  : "text-gray-600 hover:bg-gray-200"
              }`} onClick={() => setActiveTab(tab)}>
              <Icon className="text-xl mr-2" />{name}</button>
          ))}
        </div>
      </div>

      {/* Main Content */}
      <div className="mt-10 flex justify-center items-center flex-1 bg-gray-50 p-8">
